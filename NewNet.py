...
tf.flags.DEFINE_string('train_dir', './checkpoint_2_1018',
                           """Directory where to write event logs """
                           """and checkpoint.""")
tf.flags.DEFINE_string('train_image_dir', './voxel_flow_train_image/',
                           """Directory where to output images.""")
tf.flags.DEFINE_string('test_image_dir', './voxel_flow_test_image_baseline/',
                           """Directory where to output images.""")
tf.flags.DEFINE_string('subset', 'train',
                           """Either 'train' or 'validation'.""")
tf.flags.DEFINE_string('pretrained_model_checkpoint_path', '',
...
tf.flags.DEFINE_float('initial_learning_rate', 0.00001,
                          """Initial learning rate.""")
tf.flags.DEFINE_integer('training_data_step', 1, """The step used to reduce training data size""")
...
def _read_image(filename):
    ...
    return tf.cast(image_decoded, dtype=tf.float32) / 255 - 0.5
...
def train(dataset_frame1, dataset_frame2, dataset_frame3):
    ...
    with tf.Graph().as_default():
        ...
        dataset_frame1 = dataset_frame1.apply(
            tf.data.experimental.shuffle_and_repeat(buffer_size=100000, count=None, seed=1)).map(_read_image).map(
            lambda image: tf.image.random_flip_left_right(image, seed=1)).map(
            lambda image: tf.image.random_flip_up_down(image, seed=1)).map(
            lambda image: tf.image.random_crop(image, [256, 256, 1], seed=1))
        dataset_frame1 = dataset_frame1.prefetch(4)
        ...
        dataset_frame2 = dataset_frame2.apply(
            ...
            lambda image: tf.image.random_flip_up_down(image, seed=1)).map(
            lambda image: tf.image.random_crop(image, [256, 256, 1], seed=1))
        dataset_frame2 = dataset_frame2.prefetch(4)
        ...
        dataset_frame3 = dataset_frame3.apply(
            ...
            lambda image: tf.image.random_flip_up_down(image, seed=1)).map(
            lambda image: tf.image.random_crop(image, [256, 256, 1], seed=1))
        dataset_frame3 = dataset_frame3.prefetch(4)
        ...
        #print('----------------------------------------------------------------------')
        #print(input1)
        #print([-1,input1.get_shape().as_list()[1],input1.get_shape().as_list()[2],1])
        #print(type(input1))
        ...
        total_loss = 0.1*reproduction_loss4 + reproduction_loss3 + 0.1*tf.reduce_mean(tf.square(model4.flow - model3.flow * 2.0))
        ...
        # Build train directory
        if not isdir(FLAGS.train_dir):
        ...
        for step in range(0, FLAGS.max_steps):
            ...
            if batch_idx == 0:
                loss_mean_epoch_list.append(loss_mean_epoch)
                loss_mean_epoch = loss_value
            else:
                loss_mean_epoch = (loss_mean_epoch * batch_idx + loss_value) / (batch_idx + 1)
        ...
        # plot and print mean loss each epoch
        loss_mean_epoch_list.append(loss_mean_epoch)
        for i in range(1, len(loss_mean_epoch_list)):
            print('Epoch %d: mean loss = %f' % (i, loss_mean_epoch_list[i]))

        x_axis = [num for num in range(1, len(loss_mean_epoch_list))]
        plt.title('Loss Trends')
        plt.xlabel('Epoch')
        plt.ylabel('Mean loss')
        plt.plot(x_axis, loss_mean_epoch_list[1:])
        plt.savefig(os.path.join(FLAGS.train_dir, 'Loss_trends.png'))
...
def test(dataset_frame1, dataset_frame2, dataset_frame3):
    ...
    with tf.Graph().as_default():
        ...
        edge_vgg_1 = Vgg16(input_placeholder[:, :, :, :1], reuse=None)
        edge_vgg_3 = Vgg16(input_placeholder[:, :, :, 1:2], reuse=True)
...
def run(dataset_frame1, dataset_frame2, dataset_frame3):
    print(123)
if __name__ == '__main__':

    if FLAGS.subset == 'train':
        os.environ["CUDA_VISIBLE_DEVICES"] = "0"
        data_list_path_frame1 = "Training_Data_List_2/frame1.txt"
        data_list_path_frame2 = "Training_Data_List_2/frame2.txt"
        data_list_path_frame3 = "Training_Data_List_2/frame3.txt"
        ...
        data_list_path_frame1 = "data_list/denoise_list_frame1.txt"
        data_list_path_frame2 = "data_list/denoise_list_frame2.txt"
        data_list_path_frame3 = "data_list/denoise_list_frame3.txt"
